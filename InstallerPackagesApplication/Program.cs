...
namespace InstallerPackagesApplication
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (args[0] == "assembly")
            {
                Program.ChangeAssembly(args[0], args[1]);
        ...
        private static void ChangeAssembly(string path, string assemblyPath)
        {
            StringBuilder assebblyCompiler = new StringBuilder();
            var pattern = "[assembly: AssemblyVersion]";

            using (FileStream fs = new FileStream(assemblyPath + "/SharedAssembly.cs", FileMode.Open))
            {
                using (StreamReader sr = new StreamReader(fs))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        {
                            assebblyCompiler.Append("[assembly: AssemblyVersion(\"0.4.*\")]");
                        }
                        else
                        {
                            assebblyCompiler.Append(line + "\n");
                        }
                    }
                }
            }
